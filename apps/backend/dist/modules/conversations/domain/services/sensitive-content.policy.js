...
exports.SensitiveContentPolicy = void 0;
...
class SensitiveContentPolicy {
    ...
    containsUnsafeOutput(text) {
        const normalized = text.trim();
        return SENSITIVE_OUTPUT_PATTERNS.some((pattern) => pattern.test(normalized));
    }
    refusalMessage() {
        return ('Por segurança, não peça nem envie senhas, PIN, OTP ou códigos de verificação aqui. ' +
    ...
    sanitizeForLog(text) {
        ...
        masked = masked.replace(/\b(senha|password|pin|otp|token)\s*[:=]?\s*\S+/gi, '$1=***');
...
exports.SensitiveContentPolicy = SensitiveContentPolicy;
//# sourceMappingURL=sensitive-content.policy.js.map